...
class FudgeServer {
    ...
    async handleMessage(_message, _wsConnection) {
        ...
        switch (message.command) {
            ...
            case Message_js_1.FudgeNet.COMMAND.START_GAME:
                ...
                break;
            case Message_js_1.FudgeNet.COMMAND.END_GAME:
    ...
    enterRoom(_message) {
        ...
        if (this.rooms[_message.content.room]) {
            if (this.rooms[_message.content.room].private) {
                ...
                let messageClient = {
                    idRoom: _message.idRoom, command: Message_js_1.FudgeNet.COMMAND.ROOM_ENTER, idTarget: _message.idSource, content: { ingame: true }
                };
                this.dispatch(messageClient);
            }
            else {
    ...
    leaveRoom(_message) {
        ...
        client.ready = false;
        let newHost = this.rooms[_message.idRoom].clients[Object.keys(this.rooms[_message.idRoom].clients)[0]];
        newHost.ready = true;
        let messageRoom = {
